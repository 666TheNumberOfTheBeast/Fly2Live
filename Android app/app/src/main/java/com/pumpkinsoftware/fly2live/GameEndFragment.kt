...
import com.pumpkinsoftware.fly2live.configuration.Configuration.Companion.WINNER_DRAW
...
import com.pumpkinsoftware.fly2live.utils.isNewLevel
...
class GameEndFragment : Fragment() {
    ...
    private var winner = WINNER_UNDEFINED
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        score  = requireArguments().getLong("score")
        winner = requireArguments().getInt("winner")

        // Increment wins or loses & submit score once (the first time the fragment is created)
        if (savedInstanceState == null) {
            // Get events and leaderboards clients
            val mEventsClient  = Games.getEventsClient(requireContext(), account!!)
        ...
        if (isAudioEnabled) {
            ...
            if (MULTIPLAYER && winner == WINNER_PLAYER)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (MULTIPLAYER) {
            ...
            if (winner == WINNER_PLAYER) {
                ...
                gameStatusView.setTextColor(ContextCompat.getColor(requireContext(), R.color.green))
            }
            // Check if draw in multiplayer mode
            else if (winner == WINNER_DRAW) {
                gameStatusView.text = getString(R.string.draw)
                gameStatusView.setTextColor(ContextCompat.getColor(requireContext(), R.color.gray))
            }
        }
            ...
            override fun onSuccess(realm: Realm) {
                realm.executeTransactionAsync { transactionRealm ->
                    ...
                    val player = transactionRealm.where(PlayersLevelsCollection::class.java)
                        .equalTo("_id", Configuration.PLAYER_ID)
                        .findFirst()
    ...
    private fun updateUI() {
        ...
        val scoreView = requireView().findViewById<TextView>(R.id.score)
        ...
        if (winner == WINNER_PLAYER || newHighScore) {
            val shareView = requireView().findViewById<TextView>(R.id.button_share)
